...
import { compositeFoodService } from "../../services/compositeFood/compositeFoodService.js";
...
compositeFoodRoutes.put(
  "/:id",
  validate({ params: idParamSchema, body: updateCompositeFoodSchema }),
  async (req, res, next) => {
    try {
      const food = await compositeFoodService.update(req.params.id, req.body);
      if (!food) {
        res.status(404).json({ error: "Composite food not found" });
        return;
      }
